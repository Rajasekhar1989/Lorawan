<template>
  ...
  <ExportSearch :searchQuery="searchQuery" :itemsPerPage="itemsPerPage" @export="handleExport" @search="handleSearch"
    @items-per-page-change="handleItemsPerPageChange" @btnclicks="handleClicks" />
  <!-- Table Information -->
  <div class="custom-table-area">
    <table class="custom-table">
      <thead>
        <tr>
          <!-- Display only the first `columnsToShow` headers -->
          <th v-for="(header, idx) in limitedHeaders" :key="idx" @click="sort(header.sort)">
            {{ header.label }}
            <!-- <span v-if="sortKey === header.sort">{{ sortOrder === 'asc' ? '▲' : '▼' }}</span> -->
            <span class="sort" v-html="$store.state.sort"></span>
          </th>
          <!-- Optionally show "More" if there are more columns than shown -->
          <th v-if="headerInfo.length > columnsToShow">More</th>
          <th class="tblexport">Export</th>
        </tr>
      </thead>
      <tbody>
        <tr v-if="!hasData">
        ...
        <template v-else v-for="(row, rowIndex) in getPaginatedData" :key="rowIndex">
          <tr>
            <!-- Display only the first `columnsToShow` data cells -->
            <td data-label="Full Name" v-for="(header, idx) in limitedHeaders" :key="idx">

              {{ row[header.sort] }}
            </td>
            <!-- Optionally show "More" if there are more columns -->           
            ...
            <td>
              <div class="pageOptions">
                <a v-if="isview" class="view" @click="exportRow(row, 'view')" v-html="$store.state.view"></a>
                <a v-if="isedit" class="edit" @click="exportRow(row, 'edit')" v-html="$store.state.edit"></a>
                <a v-if="isdelete" class="delete" @click="exportRow(row, 'delete')" v-html="$store.state.delete"></a>
              </div>
            </td>
          </tr>
          <!-- Expand row for additional data if expanded -->
